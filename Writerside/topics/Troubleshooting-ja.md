...
| 見えたもの                                                                                  | 使うもの                                                                                                                                        |
|---------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
| ステータスバーの赤いエラーアイコン、またはこのプラグイン名が出ている IDE のエラーダイアログ | ダイアログの **Report to DevOps Lens** ボタン。下記の[クラッシュレポート](#crash-reports)を参照                                         |
| 壊れている・おかしいが、クラッシュはしていない                                              | <ui-path>Help &#124; Report DevOps Lens Issue…</ui-path>。バージョン情報が入力済みの状態でバグ報告フォームが開きます                    |
| すでに issue を書いていて、環境の詳細がほしい                                               | <ui-path>Help &#124; Copy DevOps Lens Diagnostics</ui-path>。マスキング済みのスナップショットがクリップボードに入るので、確認してから貼り付けてください |

### クラッシュレポート {id="crash-reports"}

プラグインが予期しないエラーをスローすると、IDE は標準のエラーダイアログを表示します。 **Report to DevOps Lens**
を押すと、スタックトレースが開発者の非公開エラー トラッカーへ直接送られるため、あなたが `idea.log` を掘り返さなくてもバグを特定できます。

押す価値があるのは次の理由からです。

- **押さない限り何も送信されません。** ダイアログを閉じれば、マシンから何も出ていきません。 キューもバックグラウンドでの再試行もありません。
- **送信前に最小化・マスキングされます** — サーバー側ではなく、あなたのマシン上で行われます。トークンは
  秘匿され、組織名、プロジェクト名、リポジトリ名、サーバー名、ユーザー名はプレースホルダーに
  置き換えられます。コード、差分、コメントが含まれることはありません。
- **非公開です。** レポートは公開トラッカーではなく、ドイツデータリージョンに設定された非公開 Sentry プロジェクト に送られます。
- ダイアログのコメント欄に、何をしていたかを一文添えられます。その短いメモが、スタック トレースを修正へと変えることがよくあります。
...
<ui-path>Help &#124; Copy DevOps Lens Diagnostics</ui-path> は、短くマスキングされた
...
> 。ログは既定でトークンを秘匿してキャプチャしますが、送信前に必ず再確認してください。
> {style="warning"}
