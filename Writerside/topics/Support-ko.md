...
| 이런 상황이라면                                                           | 여기로                                                                                               |
|---------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| "IDE에 빨간 오류 아이콘이나 이 플러그인에 대한 오류 대화 상자가 떴습니다" | 그 대화 상자에서 **Report to DevOps Lens**을 누르세요 — [크래시 보고서](#crash-reports) 참고 |
...
플러그인이 예기치 못한 오류를 던지면 IDE가 표준 오류 대화 상자를 표시하며, 거기에 **Report to DevOps Lens** 버튼이 있습니다. 이 버튼은 사용자가 누를 수 있는 가장 유용한
버튼입니다. 버그를 실제로 짚어 내는 부분인 스택 트레이스를, `idea.log`를 직접 찾아 읽고 지울 필요 없이 그대로 보내 주기 때문입니다.

누르지 않으면 아무것도 전송되지 않고, 전송 전에 사용자 컴퓨터에서 자격 증명과 신원을 드러내는 이름들을 걸러 내며, 코드는 절대 포함되지 않고, 공개 저장소가 아니라 비공개 오류 트래커로 갑니다. 가능하다면 대화
상자의 댓글 상자에 무엇을 하고 있었는지 한 문장 적어 주세요. 스택 트레이스에 "충돌이 있는 PR에서 Approve를 눌렀습니다" 한 줄만 더해져도 대개 무언가를 고치기에 충분합니다.

크래시 보고서에는 회신 주소나 의도적으로 할당된 계정 식별자가 없으므로 답장을 보낼 수 없습니다. Sentry가 연결 메타데이터를 처리할 수 있으며 완전한 익명성이 보장되지는 않습니다. **답을 원한다면 이슈도 함께 등록해 주세요.** 둘은 서로를 보완합니다.
...
<procedure title="IDE에서 신고하기" id="report-from-ide">
    ...
    <step>나머지 환경 정보는 <ui-path>Help | Copy DevOps Lens
        Diagnostics</ui-path>를 실행해 마스킹된 스냅샷을 검토한 뒤 이슈에 붙여 넣으세요.</step>
</procedure>
