...
| 你看到了什么                                                | 用这个                                                                                                         |
|-------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------|
| 状态栏中的红色错误图标，或者一个点名本插件的 IDE 错误对话框 | 该对话框中的 **Report to DevOps Lens** 按钮 — 参见下文的[崩溃报告](#crash-reports)                     |
| 有东西坏了或不对劲，但并没有崩溃                            | <ui-path>Help &#124; Report DevOps Lens Issue…</ui-path> — 打开缺陷表单，其中版本信息已预填            |
| 你已经在写 issue，只想要环境信息                            | <ui-path>Help &#124; Copy DevOps Lens Diagnostics</ui-path> — 把一份脱敏快照放到剪贴板上，供你检查后粘贴 |

### 崩溃报告 {id="crash-reports"}

当插件抛出了它没有预料到的错误时，IDE 会显示它标准的错误对话框。按下 **Report to DevOps Lens**
会把堆栈跟踪直接发送到开发者的私有错误追踪服务，这样 根本不需要你去翻 `idea.log`，缺陷就能被找出来。

它值得一按的理由：

- **除非你按下它，否则不会发送任何内容。** 关闭对话框，就不会有任何东西离开你的机器。 没有队列，也没有后台重试。
- **它在发送前就已最小化并脱敏** — 是在你的机器上完成的，而不是在服务器上。令牌会被脱敏，
  组织、项目、仓库、服务器和用户名会被替换为占位符。你的代码、差异和评论绝不会被包含 在内。
- **它是私有的。** 报告会发往配置在德国数据区域的私有 Sentry 项目，而不是公开的问题追踪仓库。
- 你可以在对话框的评论框里补上一句你当时在做什么 — 正是这短短一句话，常常能把一份堆栈 跟踪变成一个修复。
...
已脱敏的快照放到你的剪贴板上：插件版本、IDE 构建号、Java 运行时、操作系统、配置了多少个
